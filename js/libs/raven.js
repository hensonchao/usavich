...
TraceKit.report = (function reportModuleWrapper() {
    ...
    function report(ex) {
        ...
        if (lastExceptionStack) {
            if (lastException === ex) {
                return; // already caught by an inner catch blocked, ignore
            } else {
 ...
 * exceptions (because your catch blocked will likely be far away from the
...
TraceKit.computeStackTrace = (function computeStackTraceWrapper() {
    ...
    function computeStackTraceFromStackProp(ex) {
        ...
        var chrome = /^\s*at (?:((?:\[object object\])?\S+) )?\(?((?:file|http|https|chrome-extension):.*?):(\d+)(?::(\d+))?\)?\s*$/i,
    ...
    function computeStackTraceFromOperaMultiLineMessage(ex) {
        ...
        for (i = 2, len = lines.length; i < len; i += 2) {
            ...
            if ((parts = lineRE1.exec(lines[i]))) {
                ...
                var relativeLine = (+parts[1]); // relative to the start of the <SCRIPT> blocked
